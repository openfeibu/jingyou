...
class PageBaseResourceController extends BaseController
{
    ...
    public function index(PageRequest $request){
        ...
        return $this->response->title(trans('app.admin.panel'))
            ->view('page.'.$this->category_slug.'.index')
            ->output();
    }
    public function create(PageRequest $request)
    {
        ...
        return $this->response->title(trans('app.admin.panel'))
            ->view('page.'.$this->category_slug.'.create')
    ...
    public function store(PageRequest $request)
    {
        try {
            ...
            return $this->response->message(trans('messages.success.created', ['Module' => trans('page.'.$this->category_slug.'.name')]))
                ->code(0)
                ->status('success')
    ...
    {
        if ($page->exists) {
            $view = 'page.'.$this->category_slug.'.show';
        } else {
            $view = 'page.'.$this->category_slug.'.create';
        }
        ...
        return $this->response->title(trans('app.view') . ' ' . 'page.'.trans('page.'.$this->category_slug.'.name'))
            ->data(compact('page','category_childs'))
            ->view($view)
            ->output();
    }
    ...
    public function destroy(PageRequest $request,Page $page)
    {
        try {
            ...
            return $this->response->message(trans('messages.success.deleted', ['Module' => trans('page.'.$this->category_slug.'.name')]))
                ->status("success")
                ->code(202)
                ->url(guard_url($this->main_url))
                ->redirect();
